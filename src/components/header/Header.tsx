...
const Header = async () => {
    ...
    const tvGenres = buildGenreItems(tvGenresResponse.genres, "/tv");

    return (
        <header
            className="
                sticky top-0 z-50 w-full border-b border-[var(--color-border)]
                bg-[var(--color-card)]/90 text-[var(--color-text)]
                backdrop-blur-xl transition-colors duration-300
            "
        >
            <div
                className="
                    relative flex h-14 w-full items-center gap-3 px-3
                    sm:h-16 sm:px-5 md:px-6 lg:px-8 xl:px-10
                "
            >
                ...
                <div className="hidden w-full max-w-[360px] shrink items-center justify-end lg:flex">
                    <HeaderSearch />
                </div>
